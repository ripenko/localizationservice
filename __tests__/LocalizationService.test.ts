import LocalizationService, { ILocalizationLanguage } from "../src/LocalizationService";
...
it("Setup. importedLanguages", async () => {
    ...
    result = (await service.importLanguage({
        "Some": {
            "Key": "Some Key {0}.{1}"
        }
    })).localize("Some.Key");
...
it("Setup. onLanguageChanged", async () => {
    let changedLanguage: ILocalizationLanguage | null = null;
    ...
    await new LocalizationService({
        ...
        onLanguageChanged: async (languageName: string, language: ILocalizationLanguage): Promise<void> => {
...
it("Setup. onLanguageImported", async () => {
    ...
    let importedLanguage: ILocalizationLanguage | null = null;
    const service = new LocalizationService({
        onLanguageImported: async (name: string, language: ILocalizationLanguage) => {
...
it("Returns localized object", async () => {
    const language: ILocalizationLanguage = {
        __cultureName__: "",
        "SomeKey": {
            "Key1": "Value1",
            "Key2": "Value2"
